...
%     Dxxxx = Dxx*Dxx;
    N = N - 2;
    B = (2 * rho * A / k^2 * eye(N) + T / h^2 * Dxx(2:end-1,2:end-1) - E * I / h^4 * Dxxxx(2:end-1,2:end-1) + 2 * s1 / (k * h^2) * Dxx(2:end-1,2:end-1)) / (rho * A / k^2 + s0 / k);
    C = ((-rho * A / k^2 + s0 / k) * eye(N) - 2 * s1 / (k*h^2) * Dxx(2:end-1,2:end-1)) / (rho * A / k^2 + s0/k);
