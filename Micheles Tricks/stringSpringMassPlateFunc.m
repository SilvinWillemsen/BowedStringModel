function [ratio] = stringSpringMassPlateFunc (s0S, K1, lengthSound)
    %% Sample rate
    fs = 44100;
    k = 1/fs;
...
%     K1 = 1000;
    ...
    %         Falpha = (u1Next(cL) - u2Next + K1 * etaSpring / (2 * varPhi) + etaSpringPrev) / varPsi;
        ...
        %Total Energy
        rOCTotEnergy(n) = rOCenergy1(n) + rOCenergy2(n) + rOCenergy3(n) + (1 - s0S / (rhoS * A) * K1/1.7626e+10) * rOCconnEnergy(n) - rOCcolEnergy1(n) - rOCcolEnergy2(n); %including damping so should be 0
    ...
    plot(rOCTotEnergy(10:n));
    drawnow
    ratio = median(rOCconnEnergy(10:n)./rOCTotEnergy(10:n))
%     if ~drawThings
%         plot(out)
%     end
end