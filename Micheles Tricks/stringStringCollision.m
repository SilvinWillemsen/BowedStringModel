...
%     etaNext(vec1) = etaNextV(vec1) ./ etaNextDiv(vec1);
    for l = vec1
        Adiv = [rho1 * A1 / k^2 + g(l).^2/4, -g(l).^2/4; ...
                -g(l).^2/4, rho2 * A2 / k^2 + g(l).^2/4];
        v = [rho1 * A1 / k^2 * (2 * u1(l) - u1Prev(l)) + T1 / h1^2 * (u1(l+1) - 2 * u1(l) + u1(l-1)) ...
             - E1 * I1 / h1^4 * (u1(l+2) - 4 * u1(l+1) + 6 * u1(l) - 4 * u1(l-1) + u1(l-2))...
             + s01 / k  * u1Prev(l) ...
             - g(l).^2/4 .* etaPrev(l) + psiPrev(l) .* g(l);...
             rho2 * A2 / k^2 * (2 * u2(l) - u2Prev(l)) + T2 / h2^2 * (u2(l+1) - 2 * u2(l) + u2(l-1)) ...
             - E2 * I2 / h2^4 * (u2(l+2) - 4 * u2(l+1) + 6 * u2(l) - 4 * u2(l-1) + u2(l-2))...
             + s02 / k  * u2Prev(l) ...
             + g(l).^2/4 .* etaPrev(l) - psiPrev(l) .* g(l)];
        solut = Adiv\v;
        u1Next(l) = solut(1);
