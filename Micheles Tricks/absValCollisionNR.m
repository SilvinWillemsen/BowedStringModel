...
%         end
%     else
    g = sign(etau) * sqrt(K * (alpha1+1) / 2) * abs(etau)^((alpha1 - 1)/2);
%     end
    gSave(n) = g;
    ...
    while eps > colTol && ii < 100
        ...
        f = abs(etawNext)^(alpha1+1) - abs(etawPrev)^(alpha1+1);
        df = (alpha1+1) * abs(etawNext)^alpha1 * sign(etawNext);
        gg = etawNext - etawPrev;
        dgg = 1;
        
        coeff = (k^2 / (rho * A * hS) + k^2 / M) * K / (alpha1+1);
        
        G = etawNext + coeff * (abs(etawNext)^(alpha1+1) - abs(etawPrev)^(alpha1+1)) / (etawNext - etawPrev) + b;
    ...
    disp(ii)
    %% Calculate force and add to the string and mass-spring
    forceNR(n) = K / (alpha1 + 1) * (abs(etawNext)^(alpha1+1) - abs(etawPrev)^(alpha1+1)) / (etawNext - etawPrev);
    ...
    % Connection and Collision energies
    colEnergyw1(n) = K / (alpha1 + 1) * 0.5 * (abs(etaw)^(alpha1+1) + abs(etawPrev)^(alpha1+1));
    ...
    if mod(n,drawSpeed) == 0 && n >= drawStart && drawThings == true
        ...
        cla
        if n > 100
            plot(forceEXP(n-100:n));
            hold on
            plot(forceNR (n-100:n));
        end
        set(gca, 'Fontsize', 16)
        title("Both") 
